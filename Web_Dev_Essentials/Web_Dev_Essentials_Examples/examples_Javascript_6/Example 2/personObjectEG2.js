...
var pers = [];//Create an empty array that we will use to store all the Person objects created.
...
// [lines 17-20] check if the user useed the page before
    if (sessionStorage.getItem("hasCodeRunBefore") === null) {
       // let arrayOfPersonObjects = [];
        sessionStorage.setItem("people", JSON.stringify(pers));
        sessionStorage.setItem("hasCodeRunBefore", true);
    } else {
        pers = JSON.parse(sessionStorage.getItem("people"));//Get the array of person objects from sessionStorage and assign it to the array 'pers'
        let i = 0;
...
function addPerson() {
    pers = JSON.parse(sessionStorage.getItem("people"));
